...
@setup
  ...
  // Source Repository
  $repo                   = 'git@bitbucket.org:appkr/api-server.git';
  $branch                 = 'master';

  // Container Folder for New Release
  $release_name           = 'release_' . date('Ymd');

  // donot modify below. This is intended variable override
  $phpini_folder          = $web_server;
...
@task('migrate_db', ['on' => 'web', 'confirm' => true])
  echo "TASK: Migrating table schema";
...
@task('install_server')
  echo "SUB-TASK: Installing php, mysql, git, npm, node, memche...";
...
@task('configure_mysql')
  echo "SUB-TASK: Setting db password";
...
@task('create_db')
  echo "SUB-TASK: Creating database";
...
@task('create_db_user')
  echo "SUB-TASK: Adding new database user";
...
@task('configure_php')
  echo "SUB-TASK: Setting php. error_reporting, display_errors, ...";

  sed -i "s/error_reporting = .*/error_reporting = E_ALL & ~E_DEPRECATED & ~E_STRICT/" /etc/php5/{{ $phpini_folder }}/php.ini;
  sed -i "s/display_errors = .*/display_errors = Off/" /etc/php5/{{ $phpini_folder }}/php.ini;
  sed -i "s/cgi.fix_pathinfo=1/cgi.fix_pathinfo=0/" /etc/php5/{{ $phpini_folder }}/php.ini;
...
@task('configure_apache2')
  echo "SUB-TASK: Setting apache2. enable rewrite module, set document root, ...";
...
";
  ...
  sudo tee /etc/apache2/sites-available/000-default.conf <<<"$block";
@endtask

@task('configure_apache2_ssl')
  echo "SUB-TASK: Setting openssl certificate for apache2";
...
";
  ...
  sudo tee /etc/apache2/sites-available/default-ssl.conf <<<"$block";
@endtask

@task('configure_nginx')
  echo "SUB-TASK: Setting nginx. Set document root, ...";
...
";
  ...
  sudo tee /etc/nginx/sites-available/{{ $http_host }} <<<"$block";
  sudo ln -fs "/etc/nginx/sites-available/{{ $http_host }}" "/etc/nginx/sites-enabled/{{ $http_host }}"
@endtask

@task('install_composer')
  echo "SUB-TASK: Installing composer to /usr/local/bin/";
...
@task('install_bower_gulp')
  echo "SUB-TASK: Installing bower and gulp";

  sudo npm install -g bower gulp;
@endtask

@task('make_release_path')
  echo "SUB-TASK: Making a directory for housing of the releases";
...
@task('fetch_repo')
  echo "SUB-TASK: Pulling the latest code from git";
...
@task('set_commons')
  echo "SUB-TASK: Creating storage folder and .env file";
...
@task('run_composer')
  echo "SUB-TASK: Installing project dependencies";
...
@task('run_npm')
  echo "SUB-TASK: Installing required node packages";
...
@task('run_bower')
  echo "SUB-TASK: Installing required external resources (js/css)";
...
@task('run_gulp')
  echo "SUB-TASK: Builing resources";
...
@task('update_release_permissions')
  echo "SUB-TASK: Chaning ownership of this release to current user";

  sudo chown -R {{ $server_user }}:{{ $server_group }} {{ $releases_path }}/{{ $release_name }};
@endtask

@task('update_release_symlinks')
  echo "SUB-TASK: Linking this release to the service";
...
@task('restart_webserver')
  echo "SUB-TASK: Restarting web server";

  sudo service {{ $web_server }} restart;
@endtask

@task('restart_mysql')
  echo "SUB-TASK: Restarting database";

  sudo service mysql.server restart;
@endtask

@task('register_queue_worker')
  echo "SUB-TASK: Adding new queue subscriber";
